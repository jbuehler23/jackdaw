...
pub(crate) fn add_component_displays(
    ...
    // In Live mode the inspector mirrors the running game, not the
    // editor selection. Skip so a viewport selection doesn't paint
    // Scene component rows over the Live view.
    if *view_mode == crate::pie_mirror::PieViewMode::Live {
        return;
    }
...
pub(crate) fn on_inspector_dirty(
    ...
    // The Live view owns the inspector while it's active; its own
    // systems handle rebuilds from the mirror.
    if *view_mode == crate::pie_mirror::PieViewMode::Live {
        return;
    }
...
// ----------------------- PIE Live inspector -----------------------
//
// When `PieViewMode == Live` the inspector shows the selected game
// entity from `PieMirror` instead of the selected editor ECS entity.
// The component data is JSON keyed by type path, so it is deserialized
// onto a hidden proxy entity and rendered read-only through the same
// `build_inspector_displays` path the BRP remote inspector uses.

/// Hidden proxy entity that the selected Live mirror entity's components
/// are deserialized onto so the shared inspector renderer can display
/// them. Stored on each `Inspector` so multi-instance docks each get one.
#[derive(Component)]
pub(crate) struct PieLiveInspectorProxy(pub(crate) Entity);

/// Component ids deserialized onto the proxy for the current Live entity,
/// tracked so they can be stripped before the next rebuild.
#[derive(Component, Default)]
pub(crate) struct PieLivePopulated(Vec<bevy::ecs::component::ComponentId>);

/// Snapshot of the Live view state the inspector last rendered, used to
/// detect when a rebuild is needed.
#[derive(Component, Clone, Copy)]
pub(crate) struct PieLivePrev {
    live: bool,
    selected: Option<u64>,
}

/// Phase-2 flag: the proxy is populated and ready to render. Carries the
/// components that could not be deserialized so they render as raw JSON.
#[derive(Component)]
pub(crate) struct PieLiveInspectorNeedsRebuild {
    proxy_entity: Entity,
    fallback_components: Vec<(String, serde_json::Value)>,
}

/// Exclusive pass: when the Live view, selection, or mirror changes,
/// rebuild the proxy that backs the Live inspector. On a switch back to
/// Scene, clean up and re-trigger the normal Scene rebuild for whatever
/// entity is selected. Mirrors `remote_inspector::populate_remote_proxy`.
pub(crate) fn pie_live_inspector_populate(world: &mut World) {
    let live =
        *world.resource::<crate::pie_mirror::PieViewMode>() == crate::pie_mirror::PieViewMode::Live;
    let selected = world
        .resource::<crate::pie_mirror::PieLiveSelection>()
        .selected;
    let mirror_changed = world.is_resource_changed::<crate::pie_mirror::PieMirror>();

    let inspectors: Vec<Entity> = {
        let mut q = world.query_filtered::<Entity, With<Inspector>>();
        q.iter(world).collect()
    };

    for inspector_entity in inspectors {
        let prev = world.get::<PieLivePrev>(inspector_entity).copied();
        let view_or_selection_changed = match prev {
            Some(p) => p.live != live || p.selected != selected,
            None => live,
        };
        let mirror_only = live && mirror_changed && !view_or_selection_changed;
        let changed = view_or_selection_changed || mirror_only;
        if !changed {
            continue;
        }
        // A mirror delta must not tear down a field the user is mid-edit:
        // the input would vanish on the next frame's rebuild. Hold the
        // rebuild while one of this inspector's live fields is focused;
        // the next delta after blur/commit repaints with the confirmed
        // value. View and selection changes still rebuild (explicit user
        // navigation), and an unfocused read-only mirror still refreshes.
        if mirror_only && pie_live_input_focused(world, inspector_entity) {
            continue;
        }
        world
            .entity_mut(inspector_entity)
            .insert(PieLivePrev { live, selected });

        // Drop the components left on the proxy from the previous entity.
        pie_live_cleanup_proxy(world, inspector_entity);
        // Despawn whatever the inspector is currently showing (Scene or
        // Live); the active view repopulates below.
        pie_live_despawn_displays(world, inspector_entity);

        if !live {
            // Leaving Live: let the Scene path rebuild the selected
            // entity via the existing dirty-flag machinery.
            world.entity_mut(inspector_entity).remove::<PieLivePrev>();
            if let Some(primary) = world.resource::<Selection>().primary()
                && let Ok(mut ec) = world.get_entity_mut(primary)
            {
                ec.insert(InspectorDirty);
            }
            continue;
        ...
        let Some(bits) = selected else {
            pie_live_spawn_placeholder(world, inspector_entity, "No entity selected");
            continue;
        };

        let components: Vec<(String, serde_json::Value)> = world
        ...
        if components.is_empty() {
            pie_live_spawn_placeholder(world, inspector_entity, "No component data");
            continue;
        }

        let proxy_entity = match world.get::<PieLiveInspectorProxy>(inspector_entity) {
            Some(p) => p.0,
            None => {
                let proxy = world.spawn(crate::EditorEntity).id();
                world
                    .entity_mut(inspector_entity)
                    .insert(PieLiveInspectorProxy(proxy));
                proxy
            }
        };

        let (populated_ids, fallback_components) =
            crate::remote::remote_inspector::populate_proxy_from_components(
                world,
                proxy_entity,
                &components,
            );
        ...
        world
            .entity_mut(inspector_entity)
            .insert(PieLivePopulated(populated_ids));
        world
            .entity_mut(inspector_entity)
            .insert(PieLiveInspectorNeedsRebuild {
                proxy_entity,
                fallback_components,
            });
    }
...
/// Consume the rebuild flag and render the proxy through the shared
/// inspector builder. Fields are editable in Live mode so edits stream
/// to the running game. Mirrors `remote_inspector::build_remote_inspector_displays`.
pub(crate) fn pie_live_inspector_build(
    mut commands: Commands,
    components: &Components,
    type_registry: Res<AppTypeRegistry>,
    names: Query<&Name>,
    ...
    for (inspector_entity, rebuild) in &inspectors {
        ...
        build_inspector_displays(
            ...
            &icon_font,
            &editor_font,
            // Live fields are editable; a commit streams a SetComponent to
            // the running game (see `try_route_pie_live_field_edit`).
            false,
            &materials,
            &empty_jsn_paths,
            None,
        ...
        if !fallback_components.is_empty() {
            crate::remote::remote_inspector::spawn_fallback_section(
                &mut commands,
                inspector_entity,
                proxy_entity,
                &fallback_components,
...
/// Strip the components that the previous Live entity left on the proxy.
fn pie_live_cleanup_proxy(world: &mut World, inspector_entity: Entity) {
    let component_ids = world
        .get::<PieLivePopulated>(inspector_entity)
        .map(|p| p.0.clone())
        .unwrap_or_default();
    ...
    let Some(proxy) = world
        .get::<PieLiveInspectorProxy>(inspector_entity)
        .map(|p| p.0)
    else {
        return;
    };
    if let Ok(mut proxy_mut) = world.get_entity_mut(proxy) {
        for comp_id in component_ids {
            proxy_mut.remove_by_id(comp_id);
        }
    }
    world
        .entity_mut(inspector_entity)
        .insert(PieLivePopulated::default());
}

/// Despawn the inspector's current `ComponentDisplay` children.
fn pie_live_despawn_displays(world: &mut World, inspector_entity: Entity) {
...
fn pie_live_spawn_placeholder(world: &mut World, inspector_entity: Entity, message: &str) {
    world.spawn((
        ComponentDisplay,
...
/// Whether keyboard focus currently sits in one of this inspector's live
/// fields. Walks up from the focused entity to a [`FieldBinding`] (or the
/// `Name` field) whose source is the inspector's live proxy. Used to hold
/// the mirror-driven rebuild so a focused input is not destroyed mid-edit.
fn pie_live_input_focused(world: &World, inspector_entity: Entity) -> bool {
    let Some(proxy) = world
        .get::<PieLiveInspectorProxy>(inspector_entity)
        .map(|p| p.0)
    else {
    ...
    let Some(focused) = world.resource::<bevy::input_focus::InputFocus>().0 else {
        return false;
    };
    ...
    for _ in 0..8 {
        if world
            .get::<super::FieldBinding>(current)
            .is_some_and(|b| b.source_entity == proxy)
        {
            return true;
        }
        if world
            .get::<super::NameFieldInput>(current)
            .is_some_and(|n| n.0 == proxy)
        {
            return true;
        }
