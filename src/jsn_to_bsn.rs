...
pub fn bsn_scene_to_jsn(text: &str) -> Result<jackdaw_jsn::JsnScene, BevyError> {
    let ast = jackdaw_bsn::parse_bsn_text(text)
        .map_err(|e| BevyError::from(format!("could not parse .bsn: {e}")))?;
    ...
    while let Some((node, parent)) = queue.pop() {
        let mut entity = jackdaw_jsn::format::JsnEntity {
            id: None,
            parent,
