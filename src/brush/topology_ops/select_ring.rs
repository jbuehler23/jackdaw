...
use jackdaw_geometry::editmesh::select::ring_walk::ring_walk;
...
pub(crate) fn brush_select_ring(
    ...
    // For each selected EditMesh edge, walk its ring. Union all results.
    let mut walked: HashSet<EdgeKey> = HashSet::new();
    for ek in bmesh_edges {
        for k in ring_walk(&bmesh_component.mesh, ek) {
            walked.insert(k);
        }
    }
    if walked.is_empty() {
