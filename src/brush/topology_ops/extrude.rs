//! `brush.mesh.extrude_region` operator.

use bevy::prelude::*;
use jackdaw_api::prelude::*;
use jackdaw_geometry::bmesh::{BMesh, FaceKey};
use jackdaw_geometry::bmesh::ops::extrude_face_region::extrude_face_region;
...
pub(crate) fn brush_extrude_region(
    ...
    // Map cache face indices to BMesh FaceKeys via face_keys parallel array.
    ...
    // Run extrude_face_region on each selected face.
    // If an individual face fails (e.g. degenerate), skip silently; other faces still extrude.
    for fk in bmesh_faces {
        let _ = extrude_face_region(&mut bmesh_component.mesh, fk, DEFAULT_EXTRUDE_DEPTH);
    }
