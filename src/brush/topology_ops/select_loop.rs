//! `brush.select.loop` operator. Expands the edge selection by walking the
//! parallel-edge ring around each selected edge through quad faces.
...
#[operator(
    id = "brush.select.loop",
    label = "Loop Select",
    is_available = can_run_select_loop,
    allows_undo = false
)]
pub(crate) fn brush_select_loop(
    ...
    let new_edges = jackdaw_select::loop_edges(&halfedge.mesh, &halfedge.vert_keys, &sel_edges);
...
pub(crate) fn can_run_select_loop(
    edit_mode: Res<EditMode>,
...
pub(crate) fn add_to_extension(ctx: &mut ExtensionContext) {
    ctx.register_operator::<BrushSelectLoopOp>();
}
