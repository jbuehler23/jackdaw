...
/// How far this axis must move for one of `lines` to land on a
/// candidate: the smallest such move inside `threshold`, or zero.
fn nearest_delta(lines: [f32; 3], candidates: &[f32], threshold: f32) -> f32 {
    if threshold <= 0.0 {
        return 0.0;
    }
