...
pub fn split_edge(bmesh: &mut EditMesh, edge: EdgeKey, t: f32) -> Result<VertKey, SplitError> {
    ...
    let new_edge = bmesh.edges.insert(EditEdge {
        v: [new_vert, v1],
        flag: edge_data.flag,
        loop_first: None,
    ...
    let incident_loops: Vec<LoopKey> = crate::editmesh::cycles::radial_walk(bmesh, edge).collect();
    ...
    for lp_old in incident_loops {
        ...
        let lp_new = bmesh.loops.insert(EditLoop {
            vert: new_vert,
