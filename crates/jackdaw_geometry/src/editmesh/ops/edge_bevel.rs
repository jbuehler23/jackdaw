...
pub fn edge_bevel(
    ...
    for &edge_key in edges {
        bevel_one_edge(bmesh, edge_key, width, &mut new_faces, &mut new_edges);
    }
...
fn bevel_one_edge(
    ...
    if !bmesh.edges.contains_key(edge_key) {
        return;
    }

    // 1. Verify exactly 2 adjacent faces, each with loop_count >= 4.
    let radial: Vec<LoopKey> = radial_walk(bmesh, edge_key).collect();
    if radial.len() != 2 {
        return;
    }
    let l_a = radial[0];
    let l_b = radial[1];
    let face_a = bmesh.loops[l_a].face;
    let face_b = bmesh.loops[l_b].face;
    if bmesh.faces[face_a].loop_count < 4 || bmesh.faces[face_b].loop_count < 4 {
        return;
    }

    let v0 = bmesh.edges[edge_key].v[0];
    let v1 = bmesh.edges[edge_key].v[1];
    ...
    let outward_expected = {
        let n_a = bmesh.faces[face_a].normal_cache;
        let n_b = bmesh.faces[face_b].normal_cache;
    ...
    let v0_b = bmesh.add_vert(v0_pos + dir_b_v0 * width);
    let v1_a = bmesh.add_vert(v1_pos + dir_a_v1 * width);
    let v1_b = bmesh.add_vert(v1_pos + dir_b_v1 * width);
    ...
    collect_faces_at_vert(bmesh, v0, &mut affected);
    collect_faces_at_vert(bmesh, v1, &mut affected);
    ...
    for &fk in &affected {
        let ring = build_new_ring(
            bmesh, fk, face_a, face_b, edge_key, v0, v1, v0_a, v0_b, v1_a, v1_b,
        );
    ...
    disk_remove_edge(bmesh, edge_key);
    bmesh.edges.remove(edge_key);
    ...
    purge_edges_at_vert(bmesh, v0);
    purge_edges_at_vert(bmesh, v1);
    if bmesh.verts.contains_key(v0) {
        bmesh.verts.remove(v0);
    }
    if bmesh.verts.contains_key(v1) {
        bmesh.verts.remove(v1);
    }
    ...
    for (ring, mat) in &rebuilds {
        ...
        if let Ok(face) = create_face_from_verts_with_material(bmesh, ring, Some(*mat)) {
            ...
            let positions: Vec<bevy::math::Vec3> =
                ring.iter().map(|&k| bmesh.verts[k].co).collect();
    ...
    let candidate_positions: Vec<bevy::math::Vec3> =
        candidate_ring.iter().map(|&k| bmesh.verts[k].co).collect();
    ...
    let chamfer_mat = next_material_idx(bmesh);
    let Ok(chamfer_face) =
        create_face_from_verts_with_material(bmesh, &chamfer_ring, Some(chamfer_mat))
    else {
        return;
    };
    let positions: Vec<bevy::math::Vec3> =
        chamfer_ring.iter().map(|&k| bmesh.verts[k].co).collect();
    bmesh.faces[chamfer_face].normal_cache = newell_normal(&positions);
    new_faces.push(chamfer_face);

    // 11. Look up the two rail edges by vertex pair.
    if let Some(rail_a) = find_edge_between(bmesh, v0_a, v1_a) {
        new_edges.push(rail_a);
    }
    if let Some(rail_b) = find_edge_between(bmesh, v0_b, v1_b) {
...
/// or its `.next` does.
fn loop_at_vert(bmesh: &EditMesh, loop_on_e: LoopKey, target: VertKey) -> LoopKey {
    if bmesh.loops[loop_on_e].vert == target {
        loop_on_e
    } else {
...
fn collect_faces_at_vert(bmesh: &EditMesh, v: VertKey, out: &mut HashSet<FaceKey>) {
    for e in disk_walk(bmesh, v).collect::<Vec<_>>() {
        for lp in radial_walk(bmesh, e).collect::<Vec<_>>() {
            out.insert(bmesh.loops[lp].face);
        }
    }
}
...
fn build_new_ring(
    ...
    for _ in 0..n {
        ...
        if face == face_a {
            ...
            let prev_par_a = edge_touches_face(bmesh, prev_edge, face_a);
            let prev_par_b = edge_touches_face(bmesh, prev_edge, face_b);
            let next_par_a = edge_touches_face(bmesh, next_edge, face_a);
            let next_par_b = edge_touches_face(bmesh, next_edge, face_b);
...
fn edge_touches_face(bmesh: &EditMesh, e: EdgeKey, face: FaceKey) -> bool {
    for lp in radial_walk(bmesh, e) {
        if bmesh.loops[lp].face == face {
            return true;
        }
    }
    false
