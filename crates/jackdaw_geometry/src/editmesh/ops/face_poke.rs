...
pub fn face_poke(
    ...
    let centroid: Vec3 = ring.iter().map(|&v| bmesh.verts[v].co).sum::<Vec3>() / n as f32;
    ...
    let center_vert = bmesh.add_vert(point);
    ...
    for i in 0..n {
        ...
        let tri_face =
            create_face_from_verts_with_material(bmesh, &tri_verts, Some(original_material_idx))
    ...
    for &rv in &ring {
        let edge = find_edge_between(bmesh, center_vert, rv)
