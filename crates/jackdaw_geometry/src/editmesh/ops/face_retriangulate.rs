...
pub fn face_retriangulate(
    ...
    let ring_positions: Vec<Vec3> = ring_verts.iter().map(|&v| bmesh.verts[v].co).collect();
    ...
    if interior_points.is_empty() && constraint_edges.len() == 1 {
        ...
        if a < ring_len && b < ring_len {
            ...
            if non_adjacent {
                let va = ring_verts[a];
                let vb = ring_verts[b];
                let _new_edge = super::face_split::split_face(bmesh, face, va, vb)
                ...
                // split_face).
                let new_faces: Vec<FaceKey> = bmesh
                    .faces
                    .iter()
    ...
    // upstream).
    let new_verts: Vec<VertKey> = interior_points.iter().map(|&p| bmesh.add_vert(p)).collect();
    ...
    for poly in polygons {
        ...
        let new_face =
            create_face_from_verts_with_material(bmesh, &poly_verts, Some(original_material_idx))
                .map_err(|_| RetriangulateError::CdtFailed("face create failed".to_string()))?;
        bmesh.faces[new_face].flag = face_flag;
        bmesh.faces[new_face].normal_cache = face_normal;
