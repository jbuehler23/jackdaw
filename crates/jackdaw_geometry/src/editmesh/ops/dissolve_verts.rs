...
fn dissolve_valence_n(bmesh: &mut EditMesh, v: VertKey) -> bool {
    ...
    loop {
        ...
        match coplanar_safe_edge {
            Some(e) => {
                let _ = crate::editmesh::ops::dissolve_edges::dissolve_edges(bmesh, &[e]);
            }
            None => {
                break;
            }
    ...
    let remaining_edges: Vec<EdgeKey> = disk_walk(bmesh, v).collect();
    ...
    if remaining_edges.len() == 2 {
        return dissolve_valence_2(bmesh, v, remaining_edges[0], remaining_edges[1]);
    }

    if remaining_edges.len() == 1 {
        // Valence dropped to 1 after co-planar dissolves.
        disk_remove_edge(bmesh, remaining_edges[0]);
        bmesh.edges.remove(remaining_edges[0]);
        bmesh.verts.remove(v);
        return true;
    }
    ...
    let incident_faces = faces_incident_to(bmesh, v);
    dissolve_valence_n_fallback(bmesh, v, remaining_edges, incident_faces)
}
...
/// sets.
fn would_merge_produce_valid_face(bmesh: &EditMesh, e: EdgeKey) -> bool {
    let radial: Vec<_> = radial_walk(bmesh, e).collect();
    if radial.len() != 2 {
        return false;
    }
    let lp_a = radial[0];
    ...
    let mut cur = bmesh.loops[lp_a].next;
    while cur != lp_a {
        verts_a.insert(bmesh.loops[cur].vert);
        cur = bmesh.loops[cur].next;
    }

    // Check face_b's ring (excluding lp_b) for any overlap with face_a.
    let mut cur = bmesh.loops[lp_b].next;
    while cur != lp_b {
        if verts_a.contains(&bmesh.loops[cur].vert) {
            return false;
        }
        cur = bmesh.loops[cur].next;
    }
...
fn faces_incident_to(bmesh: &EditMesh, v: VertKey) -> HashSet<FaceKey> {
...
fn dissolve_valence_2(bmesh: &mut EditMesh, v: VertKey, e1: EdgeKey, e2: EdgeKey) -> bool {
    // Guard: both edges must still exist.
    if !bmesh.edges.contains_key(e1) || !bmesh.edges.contains_key(e2) {
        return false;
    }
    ...
    let new_edge = create_edge(bmesh, a, b);
...
fn dissolve_valence_n_fallback(
    ...
    let inherit_material_idx = incident_faces
        .iter()
        .next()
        .map(|&f| bmesh.faces[f].material_idx)
        .unwrap_or(0);
    ...
    let should_reverse = if expected_normal.length_squared() > 0.5 {
        ...
        let mut count = 0u32;
        for (k, vert) in bmesh.verts.iter() {
            if k == v {
                continue;
            }
    ...
    // Remove all incident faces (and their loops) from the EditMesh.
    for &face in &incident_faces {
        if !bmesh.faces.contains_key(face) {
            continue;
        }
    ...
    // faces so we never introduce a duplicate material_idx in the EditMesh.
    let _ = crate::editmesh::ops::face_create::create_face_from_verts_with_material(
        bmesh,
        &outer_ring,
...
fn find_loop_at_vert_in_face(bmesh: &EditMesh, face: FaceKey, vert: VertKey) -> Option<LoopKey> {
