...
pub fn inset_face(
    ...
    for i in 0..n {
        inner_loops.push(mesh.loops.insert(HalfedgeLoop {
            vert: new_verts[i],
            edge: inner_edges[i],
            face,
    ...
    mesh.faces[face].loop_first = inner_loops[0];
    mesh.faces[face].loop_count = n as u32;
    let inner_pos: Vec<Vec3> = new_verts.iter().map(|&k| mesh.verts[k].co).collect();
    mesh.faces[face].normal_cache = newell_normal(&inner_pos);
