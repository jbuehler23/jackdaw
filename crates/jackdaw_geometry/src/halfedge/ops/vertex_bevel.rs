...
pub fn vertex_bevel(
    ...
    for (e, _other) in &incident {
        for lp in radial_walk(mesh, *e).collect::<Vec<_>>() {
            adjacent_faces.insert(mesh.loops[lp].face);
        }
    }
    ...
    // 5. Allocate the offset vertices in the HalfedgeMesh. Remember which edge
    ...
    for (ring, mat) in &rebuilds {
        ...
        if let Ok(face) = create_face_from_verts_with_material(mesh, ring, Some(*mat)) {
            let positions: Vec<Vec3> = ring.iter().map(|&k| mesh.verts[k].co).collect();
    ...
    let candidate_positions: Vec<Vec3> = bevel_ring.iter().map(|&k| mesh.verts[k].co).collect();
    let candidate_normal = newell_normal(&candidate_positions);
    if candidate_normal.dot(outward) < 0.0 {
        bevel_ring.reverse();
    }
    let bevel_mat = next_material_idx(mesh);
    let bevel_face = create_face_from_verts_with_material(mesh, &bevel_ring, Some(bevel_mat))
        .map_err(|_| VertexBevelError::Degenerate)?;
    let positions: Vec<Vec3> = bevel_ring.iter().map(|&k| mesh.verts[k].co).collect();
    mesh.faces[bevel_face].normal_cache = newell_normal(&positions);
