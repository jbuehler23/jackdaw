//! Bisect an `HalfedgeMesh` by a plane.
//!
...
use crate::halfedge::ops::face_split::split_face;
use crate::halfedge::types::*;

/// What to keep after a plane bisect.
///
/// `Both` is intentionally NOT a variant: the `HalfedgeMesh` ops layer can
...
/// Bisect `mesh` along `plane`. `plane.normal` should be unit length and
...
pub fn bisect_plane(
    ...
    let mut side: HashMap<VertKey, Side> = HashMap::with_capacity(mesh.verts.len());
    for (vk, v) in mesh.verts.iter() {
    ...
    let crossing_edges: Vec<EdgeKey> = mesh
        .edges
        .iter()
        .filter_map(|(ek, e)| {
    ...
    for edge in crossing_edges {
        ...
        let p0 = mesh.verts[edge_data.v[0]].co;
        let p1 = mesh.verts[edge_data.v[1]].co;
        ...
        let new_vert = split_edge(mesh, edge, t).map_err(|_| BisectError::DegeneratePlane)?;
    ...
    // ring is non-convex.
    loop {
        let target: Option<(FaceKey, VertKey, VertKey)> = find_straddling_face(mesh, &side);
        let Some((face, va, vb)) = target else {
            break;
        };
        if split_face(mesh, face, va, vb).is_err() {
    ...
    for (fk, face) in mesh.faces.iter() {
        let mut has_front = false;
        let mut has_back = false;
        let mut cur = face.loop_first;
        for _ in 0..face.loop_count {
            let vk = mesh.loops[cur].vert;
        ...
        if has_front && !has_back {
            ...
            // by centroid sign.
            let centroid = face_centroid(mesh, fk);
    ...
    let cap_face = build_cap(mesh, &on_verts, n, flip_winding, cap_material_idx);
...
fn face_centroid(mesh: &HalfedgeMesh, fk: FaceKey) -> Vec3 {
...
fn drop_face(mesh: &mut HalfedgeMesh, fk: FaceKey) {
    if !mesh.faces.contains_key(fk) {
        return;
    }
    let face_data = mesh.faces[fk].clone();
...
fn gc_orphans(mesh: &mut HalfedgeMesh) {
    ...
    for ek in orphan_edges {
        crate::halfedge::cycles::disk_remove_edge(mesh, ek);
        mesh.edges.remove(ek);
    }
    // Verts with no incident edges.
    let orphan_verts: Vec<VertKey> = mesh
        .verts
        .iter()
...
fn build_cap(
    ...
    let live_on: HashSet<VertKey> = on_verts
        .iter()
        .copied()
        .filter(|vk| mesh.verts.contains_key(*vk))
        .collect();
    if live_on.len() < 3 {
        return None;
    }
    ...
    for (_, e) in mesh.edges.iter() {
        ...
        let radial_count = if let Some(first) = e.loop_first {
            let mut count = 1u32;
            let mut cur = mesh.loops[first].radial_next;
            while cur != first {
                count += 1;
                cur = mesh.loops[cur].radial_next;
                if count > 64 {
                    break;
                }
            }
    ...
    if ring.len() < 3 {
        ...
        ring = polar_sort_ring(mesh, &live_on, plane_normal);
        if ring.len() < 3 {
            return None;
        }
    }
    ...
    let positions: Vec<Vec3> = ring.iter().map(|&vk| mesh.verts[vk].co).collect();
    ...
    create_face_from_verts_with_material(mesh, &ring, Some(material_idx)).ok()
}
...
fn polar_sort_ring(
    ...
    let mut ranked: Vec<(VertKey, f32)> = on
        .iter()
        .map(|&vk| {
            let p = mesh.verts[vk].co - centroid;
