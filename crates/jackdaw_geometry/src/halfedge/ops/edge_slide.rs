...
pub fn edge_slide(
    ...
    for &edge in edges {
        let quad_loops: Vec<LoopKey> = radial_walk(mesh, edge)
        ...
        let v_start = mesh.loops[chosen].vert;
        let v_end = mesh.loops[mesh.loops[chosen].next].vert;

        // Slide-along edge for v_start: the edge incident to v_start in this
        // face that isn't `edge`. That's lp.prev.edge.
        let prev_loop = mesh.loops[chosen].prev;
        let slide_edge_for_start = mesh.loops[prev_loop].edge;
        if !edge_set.contains(&slide_edge_for_start) {
            let other = {
                let e = &mesh.edges[slide_edge_for_start];
                if e.v[0] == v_start { e.v[1] } else { e.v[0] }
            };
            let dir = mesh.verts[other].co - mesh.verts[v_start].co;
            vert_dirs.entry(v_start).or_default().push(dir);
        }

        // Slide-along edge for v_end: lp.next.edge.
        let next_loop = mesh.loops[chosen].next;
        let slide_edge_for_end = mesh.loops[next_loop].edge;
        if !edge_set.contains(&slide_edge_for_end) {
            let other = {
                let e = &mesh.edges[slide_edge_for_end];
                if e.v[0] == v_end { e.v[1] } else { e.v[0] }
            };
            let dir = mesh.verts[other].co - mesh.verts[v_end].co;
