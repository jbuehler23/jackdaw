...
pub fn bridge_edge_loops(
    ...
    let ring_a = walk_edges_to_ring(mesh, edges_a)?;
    let ring_b = walk_edges_to_ring(mesh, edges_b)?;
    if ring_a.len() != ring_b.len() {
        return Err(BridgeError::UnequalVertexCounts);
    }
    let n = ring_a.len();
    let edges_before: HashSet<EdgeKey> = mesh.edges.keys().collect();
    let mut new_faces: Vec<FaceKey> = Vec::with_capacity(n);
    for i in 0..n {
        let i_next = (i + 1) % n;
        let quad = [ring_a[i], ring_a[i_next], ring_b[i_next], ring_b[i]];
        let face = create_face_from_verts(mesh, &quad).map_err(|_| BridgeError::FaceCreate)?;
        new_faces.push(face);
    }
    let new_edges: Vec<EdgeKey> = mesh
        .edges
        .keys()
...
fn walk_edges_to_ring(mesh: &HalfedgeMesh, edges: &[EdgeKey]) -> Result<Vec<VertKey>, BridgeError> {
