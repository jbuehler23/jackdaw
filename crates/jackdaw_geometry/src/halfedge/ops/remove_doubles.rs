...
pub fn remove_doubles(mesh: &mut HalfedgeMesh, distance: f32) -> Result<MergeResult, MergeError> {
    if !distance.is_finite() || distance < 0.0 {
        return Err(MergeError::InvalidDistance);
    }
    if distance == 0.0 || mesh.vert_count() < 2 {
    ...
    let mut keyed: Vec<(VertKey, bevy_math::Vec3)> =
    ...
    // Fourth, flatten current HalfedgeMesh to topology.
    let topology_before = mesh.flatten_to_topology();

    // Build reverse lookup: topology vertex index -> VertKey.
    // flatten_to_topology iterates mesh.verts in slotmap iter() order, which may
    ...
    // Finally, rebuild HalfedgeMesh from cleaned topology.
