...
pub fn vertex_slide(
    ...
    // remaining slides.
    let mut moves: Vec<(VertKey, glam::Vec3)> = Vec::with_capacity(verts.len());
    for &v in verts {
        ...
        let edge = &mesh.edges[first_edge];
        let other_vert_key = if edge.v[0] == v { edge.v[1] } else { edge.v[0] };
        let v_start = vert.co;
        let target = mesh.verts[other_vert_key].co;
