...
pub fn connect_verts(
    ...
    for face in face_keys {
        ...
        let face_data = match mesh.faces.get(face) {
            Some(f) => f.clone(),
        ...
        // target face for each pair before calling split_face.
        for window in on_face.windows(2) {
            ...
            match split_face(mesh, target_face, va, vb) {
                Ok(new_edge) => {
                    ...
                    // split_face inserts exactly one new face per successful call,
                    // but we discover it by scanning rather than relying on that detail.
                    let face_count_after = mesh.faces.len();
                    if face_count_after > face_count_before {
                        ...
                        let pre_keys: HashSet<FaceKey> =
                            mesh.faces.keys().take(face_count_before).collect();
                        for (fk, _) in mesh.faces.iter() {
...
fn find_face_with_verts(mesh: &HalfedgeMesh, va: VertKey, vb: VertKey) -> Option<FaceKey> {
