...
pub fn split_face(
    ...
    let pos_a = ring_loops.iter().position(|&k| mesh.loops[k].vert == va);
    let pos_b = ring_loops.iter().position(|&k| mesh.loops[k].vert == vb);
    ...
    let new_face = mesh.faces.insert(HalfedgeFace {
        flag: face_data.flag,
    ...
    let bridge_orig = mesh.loops.insert(HalfedgeLoop {
        vert: vb,
        edge: new_edge,
        face,
    ...
    let bridge_new = mesh.loops.insert(HalfedgeLoop {
        vert: va,
        edge: new_edge,
        face: new_face,
        next: LoopKey::default(),
    ...
    mesh.loops[bridge_orig].next = forward_loops[0];
    mesh.loops[bridge_orig].prev = last_fwd;
    ...
    mesh.loops[bridge_new].next = backward_loops[0];
    mesh.loops[bridge_new].prev = last_back;

    // -----------------------------------------------------------------------
    // Reassign backward loops' face pointer to new_face, and update bridge loops.
    // -----------------------------------------------------------------------
    for &lp in &backward_loops {
        mesh.loops[lp].face = new_face;
    }
    ...
    // -----------------------------------------------------------------------
    mesh.faces[face].loop_first = forward_loops[0];
    mesh.faces[face].loop_count = (forward_loops.len() + 1) as u32;
    mesh.faces[new_face].loop_first = backward_loops[0];
    mesh.faces[new_face].loop_count = (backward_loops.len() + 1) as u32;

    // Insert both bridge loops into new_edge's radial cycle.
    radial_insert_loop(mesh, bridge_orig);
    radial_insert_loop(mesh, bridge_new);
