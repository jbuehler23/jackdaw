...
pub fn loop_cut(
    ...
    let walk = walk_loop_cut_ring(mesh, start_edge);
    if walk.is_empty() {
    ...
    let start_v0 = mesh.verts[mesh.edges[start_edge].v[0]].co;
    let start_v1 = mesh.verts[mesh.edges[start_edge].v[1]].co;
    ...
    for &edge in &edges_to_cut {
        let v0_pos = mesh.verts[mesh.edges[edge].v[0]].co;
        let v1_pos = mesh.verts[mesh.edges[edge].v[1]].co;
        ...
        let v_new = split_edge(mesh, edge, oriented_t).map_err(LoopCutError::EdgeSplit)?;
    ...
    for hop in &walk {
        ...
        let before: HashSet<FaceKey> = mesh.faces.keys().collect();
        let new_edge = split_face(mesh, hop.face, va, vb).map_err(LoopCutError::FaceSplit)?;
        let after: HashSet<FaceKey> = mesh.faces.keys().collect();
...
fn walk_loop_cut_ring(mesh: &HalfedgeMesh, start_edge: EdgeKey) -> Vec<FaceHop> {
    ...
    for start_loop in initial_loops {
        ...
        loop {
            let face = mesh.loops[entry_loop].face;
            ...
            let next1 = mesh.loops[entry_loop].next;
            let exit_loop_key = mesh.loops[next1].next; // loop at C
            let exit_edge = mesh.loops[exit_loop_key].edge;
            ...
            let radial_other = mesh.loops[exit_loop_key].radial_next;
