...
pub fn loop_cut(
    ...
    for &edge in &edges_to_split {
        let v_new = bm_edge_split(bmesh, edge, t).map_err(LoopCutError::EdgeSplit)?;
    ...
    for hop in &walk {
        ...
        let new_edge = bm_face_split(bmesh, hop.face, va, vb).map_err(LoopCutError::FaceSplit)?;
...
fn walk_loop_cut_ring(bmesh: &BMesh, start_edge: EdgeKey) -> Vec<FaceHop> {
