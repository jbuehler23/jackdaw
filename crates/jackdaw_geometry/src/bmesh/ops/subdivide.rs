...
//! Instead, two sequential `bm_face_split` calls are made, producing 3 sub-faces.
//! Full 2x2 subdivision is deferred until `bm_face_poke` lands.

use std::collections::{HashMap, HashSet};

use crate::bmesh::cycles::radial_walk;
use crate::bmesh::ops::edge_split::bm_edge_split;
use crate::bmesh::ops::face_split::bm_face_split;
use crate::bmesh::types::*;

#[derive(Debug)]
pub enum SubdivideError {
    EdgeSplit(crate::bmesh::ops::edge_split::SplitError),
    FaceSplit(crate::bmesh::ops::face_split::FaceSplitError),
}
...
pub fn subdivide(
    ...
    for &edge in edges_to_cut {
        ...
        let v_new = bm_edge_split(bmesh, edge, 0.5).map_err(SubdivideError::EdgeSplit)?;
        ...
        match cuts {
            ...
            1 => {
                ...
                if let Some(opp) = find_opposite_ring_vert(bmesh, face, mid) {
                    ...
                    match bm_face_split(bmesh, face, mid, opp) {
                        Ok(new_edge) => {
            ...
            2 => {
                ...
                // then connect them. bm_face_split handles both cases (returns
                // Adjacent error if they happen to be adjacent on the now-expanded ring,
                // which we silently skip).
                let before_faces: HashSet<_> = bmesh.faces.keys().collect();
                match bm_face_split(bmesh, face, midpoint_verts[0], midpoint_verts[1]) {
                    Ok(new_edge) => {
                        new_edges_out.push(new_edge);
                        let after_faces: HashSet<_> = bmesh.faces.keys().collect();
                        new_faces_out.extend(after_faces.difference(&before_faces).copied());
            ...
            4 => {
                ...
                // positions) and do two bm_face_split calls.
            ...
            _ => {
                ...
                for &other in &midpoint_verts[1..] {
                    ...
                    let before_faces: HashSet<_> = bmesh.faces.keys().collect();
                    match bm_face_split(bmesh, target_face, pivot, other) {
                        Ok(new_edge) => {
                            new_edges_out.push(new_edge);
                            let after_faces: HashSet<_> = bmesh.faces.keys().collect();
                            let added: Vec<_> = after_faces.difference(&before_faces).copied().collect();
...
fn find_opposite_ring_vert(bmesh: &BMesh, face: FaceKey, vert: VertKey) -> Option<VertKey> {
...
/// irregular.
fn find_opposite_pairs(bmesh: &BMesh, face: FaceKey, midpoints: &[VertKey]) -> Vec<(VertKey, VertKey)> {
    let f = &bmesh.faces[face];
    let mut ring_verts: Vec<VertKey> = Vec::with_capacity(f.loop_count as usize);
    let mut cur = f.loop_first;
    for _ in 0..f.loop_count {
...
fn find_any_face_with_verts(bmesh: &BMesh, verts: &[VertKey]) -> Option<FaceKey> {
...
fn face_ring_contains(bmesh: &BMesh, face: FaceKey, va: VertKey, vb: VertKey) -> bool {
    let f = &bmesh.faces[face];
