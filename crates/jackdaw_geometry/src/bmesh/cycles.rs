//! Disk and radial cycle maintenance for BMesh.
//!
...
//!
//! These cycles give half-edge meshes O(1) adjacency walks. All BMesh
...
fn disk_side(edge: &BMEdge, vert: VertKey) -> usize {
    if edge.v[0] == vert {
        0
...
pub fn disk_insert_edge(bmesh: &mut BMesh, e: EdgeKey) {
    for side in 0..2 {
...
pub fn disk_remove_edge(bmesh: &mut BMesh, e: EdgeKey) {
    for side in 0..2 {
        let v_key = bmesh.edges[e].v[side];
...
pub fn disk_walk(bmesh: &BMesh, v: VertKey) -> impl Iterator<Item = EdgeKey> + '_ {
...
pub fn radial_insert_loop(bmesh: &mut BMesh, lp: LoopKey) {
...
pub fn radial_remove_loop(bmesh: &mut BMesh, lp: LoopKey) {
    let e = bmesh.loops[lp].edge;
...
pub fn radial_walk(bmesh: &BMesh, e: EdgeKey) -> impl Iterator<Item = LoopKey> + '_ {
