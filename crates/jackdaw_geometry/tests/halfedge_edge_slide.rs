...
fn slide_loop_cut_edge_at_t_pos_1_moves_endpoint_toward_top() {
    ...
    let cut_result = loop_cut(&mut mesh, any_vertical_edge, 0.5).expect("cut");
    ...
    for &edge in &new_loop_edges {
        ...
        endpoint_zs_before.push(mesh.verts[e.v[0]].co.z);
        endpoint_zs_before.push(mesh.verts[e.v[1]].co.z);
    }
    ...
    edge_slide(&mut mesh, &new_loop_edges, 0.5).expect("slide");

    let mut endpoint_zs_after: Vec<f32> = Vec::new();
    for &edge in &new_loop_edges {
        let e = &mesh.edges[edge];
        endpoint_zs_after.push(mesh.verts[e.v[0]].co.z);
        endpoint_zs_after.push(mesh.verts[e.v[1]].co.z);
    }
    ...
    mesh.validate().expect("valid after slide");
}
...
fn slide_at_t_zero_does_not_move_anything() {
    ...
    let positions_before: Vec<Vec3> = mesh.verts.values().map(|v| v.co).collect();
    edge_slide(&mut mesh, &[any_edge], 0.0).expect("slide");
    let positions_after: Vec<Vec3> = mesh.verts.values().map(|v| v.co).collect();
