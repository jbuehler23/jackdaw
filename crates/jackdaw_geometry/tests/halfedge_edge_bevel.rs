...
fn bevel_cube_edge_creates_chamfer_quad() {
    ...
    let result = edge_bevel(&mut mesh, &[edge], 0.1).expect("bevel one cube edge");
...
fn bevel_zero_width_returns_error() {
    ...
    let err = edge_bevel(&mut mesh, &[edge], 0.0).expect_err("zero width is rejected");
...
fn bevel_empty_returns_error() {
    let brush = Brush::cuboid(1.0, 1.0, 1.0);
    let mut mesh = HalfedgeMesh::lift_from_topology(&brush.topology);
    let err = edge_bevel(&mut mesh, &[], 0.1).expect_err("empty input is rejected");
...
fn bevel_cube_chamfer_is_a_parallelogram() {
    ...
    let width = 0.3_f32;
    let result = edge_bevel(&mut mesh, &[edge], width).expect("bevel");

    let chamfer = result.new_faces[0];
    let mut ring: Vec<Vec3> = Vec::new();
    let face = &mesh.faces[chamfer];
    let mut cur = face.loop_first;
    for _ in 0..face.loop_count {
        ring.push(mesh.verts[mesh.loops[cur].vert].co);
...
fn bevel_every_cube_edge_is_a_parallelogram() {
    ...
    for edge_idx in 0..12 {
        ...
        let result = edge_bevel(&mut mesh, &[edge], 0.25).expect("bevel");
        let chamfer = result.new_faces[0];
...
fn bevel_adjacent_to_existing_chamfer_is_still_a_parallelogram() {
    ...
    let first_edge = mesh.edges.keys().next().expect("cube has edges");
    let first_verts = mesh.edges[first_edge].v;
    let _r1 = edge_bevel(&mut mesh, &[first_edge], 0.2).expect("first bevel");
    ...
    let touching: Option<jackdaw_geometry::halfedge::EdgeKey> = mesh
        .edges
        .iter()
        .find(|(_, e)| {
            (e.v[0] == first_verts[0]
    ...
    let r2 = edge_bevel(&mut mesh, &[second_edge], 0.2).expect("second bevel");
...
fn bevel_preserves_face_count_plus_new_chamfer() {
    ...
    let first = mesh.edges.keys().next().expect("at least one edge");
    let first_verts = mesh.edges[first].v;
    let second = mesh
        .edges
        .iter()
        .find(|(k, e)| {
    ...
    let result = edge_bevel(&mut mesh, &[first, second], 0.1).expect("bevel two cube edges");
