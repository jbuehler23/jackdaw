use jackdaw_geometry::bmesh::{BMesh, ops::edge_split::bm_edge_split};
...
fn split_edge_of_cube_adds_one_vert_one_edge_one_loop_per_adjacent_face() {
    ...
    let new_vert = bm_edge_split(&mut bmesh, edge_to_split, 0.5).expect("split");
...
fn split_edge_at_t_quarter_places_vert_at_quarter_position() {
    ...
    let new_vert = bm_edge_split(&mut bmesh, edge_to_split, 0.25).expect("split");
...
fn split_edge_then_validate_round_trip_preserves_topology_via_flatten() {
    ...
    let mut bmesh = BMesh::lift_from_topology(&brush.topology);
    let edge_to_split = bmesh.edges.keys().next().unwrap();
    bm_edge_split(&mut bmesh, edge_to_split, 0.5).unwrap();
