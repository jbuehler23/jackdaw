...
fn vertex_slide_at_t_0_5_moves_vert_halfway_to_neighbor() {
    ...
    let v_start = mesh.verts[v].co;
    // Find the first edge in this vert's disk cycle.
    let first_edge = mesh.verts[v].edge.expect("vert has incident edge");
    let other_vert = {
        let e = &mesh.edges[first_edge];
        if e.v[0] == v { e.v[1] } else { e.v[0] }
    };
    let target_pos = mesh.verts[other_vert].co;
    let expected = v_start.lerp(target_pos, 0.5);

    vertex_slide(&mut mesh, &[v], 0.5).expect("slide");
    let v_new = mesh.verts[v].co;
    assert!(
...
fn vertex_slide_at_t_zero_does_nothing() {
    ...
    let v = mesh.verts.keys().next().unwrap();
    let v_start = mesh.verts[v].co;
    vertex_slide(&mut mesh, &[v], 0.0).expect("slide");
    let v_after = mesh.verts[v].co;
...
fn vertex_slide_isolated_vert_with_no_edge_no_op() {
    ...
    let v = mesh.add_vert(Vec3::ZERO);
    // No edge on this vert.
    let result = vertex_slide(&mut mesh, &[v], 0.5);
