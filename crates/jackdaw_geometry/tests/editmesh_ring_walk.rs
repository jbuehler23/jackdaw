use jackdaw_geometry::editmesh::{EditMesh, select::ring_walk::ring_walk};
...
fn ring_walk_around_cube_returns_4_edges() {
    ...
    let result = ring_walk(&bmesh, any_edge);
    // Cube edge: ring of 4 perpendicular edges (the verticals around a cube).
    assert_eq!(result.len(), 4, "ring walk on cube should produce 4 edges, got {}", result.len());
...
fn ring_walk_returns_different_edges_than_loop_walk() {
    ...
    let bmesh = EditMesh::lift_from_topology(&brush.topology);
    let any_edge = bmesh.edges.keys().next().unwrap();
    let loop_result: std::collections::HashSet<_> = loop_walk(&bmesh, any_edge).into_iter().collect();
    let ring_result: std::collections::HashSet<_> = ring_walk(&bmesh, any_edge).into_iter().collect();
    ...
    let intersection: std::collections::HashSet<_> = loop_result.intersection(&ring_result).copied().collect();
    assert_eq!(intersection.len(), 1, "loop and ring should only share the start edge");
}
