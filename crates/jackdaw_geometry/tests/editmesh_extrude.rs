use bevy::math::Vec3;
use jackdaw_geometry::editmesh::{EditMesh, ops::extrude_face_region::extrude_face_region};
...
fn extrude_one_quad_face_of_cube_adds_4_verts_8_edges_4_faces() {
    ...
    let result = extrude_face_region(&mut bmesh, face, 0.5).expect("extrude");
    assert_eq!(bmesh.vert_count(), initial_verts + 4);
    assert_eq!(bmesh.edge_count(), initial_edges + 8);
    assert_eq!(bmesh.face_count(), initial_faces + 4);
    bmesh.validate().expect("valid after extrude");
...
fn extrude_top_face_by_1_unit_makes_cube_taller_by_1() {
    ...
    let result = extrude_face_region(&mut bmesh, top_face, 1.0).expect("extrude");
