...
fn vertex_bevel_cube_corner_creates_triangle_face() {
    ...
    let result = vertex_bevel(&mut bmesh, vert, 0.2).expect("bevel one cube corner");
    ...
    let bevel_face = &bmesh.faces[result.new_face];
    assert_eq!(
    ...
    bmesh
        .validate()
        .expect("EditMesh invariants hold after vertex bevel");
}
...
fn vertex_bevel_zero_width_errors() {
    let brush = Brush::cuboid(1.0, 1.0, 1.0);
    let mut bmesh = EditMesh::lift_from_topology(&brush.topology);
    let vert = bmesh.verts.keys().next().unwrap();
    let err = vertex_bevel(&mut bmesh, vert, 0.0).expect_err("zero width is rejected");
...
fn vertex_bevel_normal_faces_outward() {
    ...
    let mut adj_faces: HashSet<jackdaw_geometry::editmesh::FaceKey> = HashSet::new();
    for e in disk_walk(&bmesh, vert).collect::<Vec<_>>() {
        for lp in radial_walk(&bmesh, e).collect::<Vec<_>>() {
            adj_faces.insert(bmesh.loops[lp].face);
        }
    }
    ...
    let result = vertex_bevel(&mut bmesh, vert, 0.3).expect("bevel one cube corner");
    let normal = bmesh.faces[result.new_face].normal_cache;
...
fn vertex_bevel_triangle_is_planar() {
    ...
    let vert = bmesh
        ...
        .next()
        .expect("cube has at least one vert");
    let result = vertex_bevel(&mut bmesh, vert, 0.25).expect("bevel one cube corner");

    assert_eq!(result.new_verts.len(), 3, "cube corner -> 3 offset verts");
    let a = bmesh.verts[result.new_verts[0]].co;
    let b = bmesh.verts[result.new_verts[1]].co;
    let c = bmesh.verts[result.new_verts[2]].co;
...
fn vertex_bevel_preserves_adjacent_face_count() {
    ...
    let mut seen: HashSet<jackdaw_geometry::editmesh::FaceKey> = HashSet::new();
    for e in disk_walk(&bmesh, vert).collect::<Vec<_>>() {
        for lp in radial_walk(&bmesh, e).collect::<Vec<_>>() {
            let fk = bmesh.loops[lp].face;
            if seen.insert(fk) {
                adj_face_mats.push(bmesh.faces[fk].material_idx);
            }
        }
    }
    assert_eq!(adj_face_mats.len(), 3, "cube corner has 3 adjacent faces");

    let _result = vertex_bevel(&mut bmesh, vert, 0.2).expect("bevel one cube corner");
    ...
    let post_mats: HashSet<u32> = bmesh.faces.values().map(|f| f.material_idx).collect();
