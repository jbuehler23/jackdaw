...
fn face_ring(bmesh: &EditMesh, face: FaceKey) -> Vec<VertKey> {
...
fn has_edge_between(bmesh: &EditMesh, va: VertKey, vb: VertKey) -> bool {
    bmesh
        .edges
        .iter()
...
fn retriangulate_face_with_one_interior_point() {
    ...
    let top_face = face_by_idx(&bmesh, 2);
    let original_material = bmesh.faces[top_face].material_idx;

    let result = face_retriangulate(&mut bmesh, top_face, vec![Vec3::new(0.0, 1.0, 0.0)], vec![])
        .expect("retriangulate succeeds");
    bmesh.validate().expect("valid after retriangulate");

    // Steiner verts list contains exactly the one interior point.
    assert_eq!(result.new_verts.len(), 1);
    let center = result.new_verts[0];
    assert!(
        (bmesh.verts[center].co - Vec3::new(0.0, 1.0, 0.0)).length() < 1e-6,
    ...
    for fk in &result.new_faces {
        assert_eq!(
            bmesh.faces[*fk].material_idx, original_material,
...
fn retriangulate_face_with_one_constraint_edge() {
    ...
    let top_face = face_by_idx(&bmesh, 2);
    let ring = face_ring(&bmesh, top_face);
    assert_eq!(ring.len(), 4, "+Y face is a quad");
    // Constraint between ring[0] and ring[2] (a diagonal).
    let result = face_retriangulate(&mut bmesh, top_face, vec![], vec![(0, 2)])
        .expect("retriangulate succeeds");
    bmesh.validate().expect("valid after retriangulate");
    ...
    assert!(
        has_edge_between(&bmesh, ring[0], ring[2]),
...
fn retriangulate_straight_chord_yields_two_quads() {
    ...
    let split_a = split_edge(&mut bmesh, edge_01, 0.5).expect("split edge 0-1");

    // Re-lookup edge (ring[2], ring[3]) after the first split (the
    // EditMesh edge set is a slotmap; the second edge handle is
    ...
    let split_b = split_edge(&mut bmesh, edge_23, 0.5).expect("split edge 2-3");
    ...
    let top_face = face_by_idx(&bmesh, 2);
    let new_ring = face_ring(&bmesh, top_face);
    ...
    let pre_vert_count = bmesh.vert_count();
    let result = face_retriangulate(&mut bmesh, top_face, vec![], vec![(idx_a, idx_b)])
        .expect("retriangulate succeeds");
    bmesh.validate().expect("valid after retriangulate");
    ...
    assert!(
        has_edge_between(&bmesh, split_a, split_b),
...
fn retriangulate_zigzag_aggressive_merge() {
    ...
    let top_face = face_by_idx(&bmesh, 2);
    let ring_len = bmesh.faces[top_face].loop_count as usize;
    ...
    let result = face_retriangulate(&mut bmesh, top_face, vec![p0, p1, p2, p3], constraints)
        .expect("retriangulate succeeds");
    bmesh.validate().expect("valid after retriangulate");
    ...
    assert!(has_edge_between(&bmesh, v0, v1), "zigzag edge (v0, v1)");
    assert!(has_edge_between(&bmesh, v1, v2), "zigzag edge (v1, v2)");
    assert!(has_edge_between(&bmesh, v2, v3), "zigzag edge (v2, v3)");
}
...
fn retriangulate_closed_quad_loop_yields_inside_and_outside() {
    ...
    assert!(has_edge_between(&bmesh, v0, v1), "loop edge (v0, v1)");
    assert!(has_edge_between(&bmesh, v1, v2), "loop edge (v1, v2)");
    assert!(has_edge_between(&bmesh, v2, v3), "loop edge (v2, v3)");
    assert!(has_edge_between(&bmesh, v3, v0), "loop edge (v3, v0)");
...
fn retriangulate_zigzag_keeps_path_edges_as_constraint() {
    ...
    assert!(
        has_edge_between(&bmesh, v0, v1),
        "zigzag edge (v0, v1) preserved"
    );
    assert!(
        has_edge_between(&bmesh, v1, v2),
        "zigzag edge (v1, v2) preserved"
    );
    assert!(
        has_edge_between(&bmesh, v2, v3),
...
fn retriangulate_cube_top_with_zigzag_path() {
    ...
    assert!(has_edge_between(&bmesh, v0, v1), "polyline edge (v0, v1)");
    assert!(has_edge_between(&bmesh, v1, v2), "polyline edge (v1, v2)");
    assert!(has_edge_between(&bmesh, v2, v3), "polyline edge (v2, v3)");
...
fn retriangulate_preserves_material_idx() {
    ...
    let face = face_by_idx(&bmesh, 5);
    let expected_material = bmesh.faces[face].material_idx;
    assert_eq!(
        expected_material, 5,
    ...
    let result = face_retriangulate(&mut bmesh, face, vec![Vec3::new(0.0, 0.0, -1.0)], vec![])
        .expect("retriangulate succeeds");
    bmesh.validate().expect("valid after retriangulate");

    for &fk in &result.new_faces {
        assert_eq!(
            bmesh.faces[fk].material_idx, expected_material,
...
fn knife_two_points_on_front_face_produces_chord() {
    ...
    let i0 = ring_len;
    let i1 = ring_len + 1;
    let result = face_retriangulate(&mut bmesh, top_face, vec![p0, p1], vec![(i0, i1)])
