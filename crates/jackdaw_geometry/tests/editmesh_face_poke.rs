...
fn face_with_normal(bmesh: &EditMesh, target: Vec3) -> FaceKey {
    bmesh
        .faces
        .iter()
...
fn ring_verts(bmesh: &EditMesh, face: FaceKey) -> Vec<jackdaw_geometry::editmesh::VertKey> {
    let mut out = Vec::new();
...
fn poke_cube_top_face_creates_4_triangles() {
    ...
    let top = face_with_normal(&bmesh, Vec3::Y);
    let original_ring = ring_verts(&bmesh, top);
    assert_eq!(original_ring.len(), 4);

    let result = face_poke(&mut bmesh, top, Vec3::new(0.0, 1.0, 0.0)).expect("poke");

    // The poked face is gone; 4 fan triangles replace it.
    // Net face count: 6 - 1 + 4 = 9.
    assert_eq!(bmesh.face_count(), initial_faces + 3, "9 faces total");
    ...
    assert_eq!(bmesh.vert_count(), initial_verts + 1, "9 vertices");
    // Original 12 edges + 4 new spokes.
    assert_eq!(bmesh.edge_count(), initial_edges + 4, "16 edges");

    // Center vert is at the poke point.
    assert!(bmesh.verts[result.center_vert].co.distance(Vec3::Y) < 1e-5);

    // Each new face is a triangle.
    for &f in &result.new_faces {
        assert_eq!(bmesh.faces[f].loop_count, 3, "fan face is a triangle");
    }
    ...
    bmesh.validate().expect("post-poke valid");
}
...
fn poke_off_plane_errors() {
    ...
    let result = face_poke(&mut bmesh, top, Vec3::new(0.0, 5.0, 0.0));
    assert!(
...
fn poke_degenerate_face_errors() {
    ...
    let face = bmesh.faces.insert(jackdaw_geometry::editmesh::EditFace {
        flag: jackdaw_geometry::editmesh::FaceFlag::empty(),
        material_idx: 0,
        loop_first: jackdaw_geometry::editmesh::LoopKey::default(),
        loop_count: 2,
        normal_cache: Vec3::Z,
    });
    ...
    let result = face_poke(&mut bmesh, face, Vec3::ZERO);
    assert!(
...
fn poke_preserves_face_material_idx() {
    ...
    let mut bmesh = EditMesh::lift_from_topology(&brush.topology);
    let top = face_with_normal(&bmesh, Vec3::Y);
    let original_material_idx = bmesh.faces[top].material_idx;
    ...
    for &f in &result.new_faces {
        assert_eq!(
            bmesh.faces[f].material_idx, original_material_idx,
...
fn poke_preserves_invariants() {
    ...
    let mut bmesh = EditMesh::lift_from_topology(&brush.topology);
    let top = face_with_normal(&bmesh, Vec3::Y);
    face_poke(&mut bmesh, top, Vec3::new(0.0, 1.0, 0.0)).expect("poke");
    bmesh.validate().expect("post-poke validate ok");
}
