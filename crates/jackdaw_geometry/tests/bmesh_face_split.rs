use jackdaw_geometry::bmesh::{BMesh, ops::face_split::bm_face_split};
use jackdaw_jsn::Brush;

fn first_face_with_count_4(bmesh: &BMesh) -> jackdaw_geometry::bmesh::FaceKey {
    bmesh.faces.iter().find(|(_, f)| f.loop_count == 4).map(|(k, _)| k).unwrap()
}

fn ring_verts(bmesh: &BMesh, face: jackdaw_geometry::bmesh::FaceKey) -> Vec<jackdaw_geometry::bmesh::VertKey> {
    let mut out = Vec::new();
...
fn split_quad_face_along_diagonal_makes_two_tris() {
    ...
    let _new_edge = bm_face_split(&mut bmesh, face, ring[0], ring[2]).expect("split");
...
fn split_with_adjacent_verts_errors() {
    ...
    let result = bm_face_split(&mut bmesh, face, ring[0], ring[1]);
...
fn split_with_off_face_vert_errors() {
    ...
    let result = bm_face_split(&mut bmesh, face, ring[0], off_face_vert);
