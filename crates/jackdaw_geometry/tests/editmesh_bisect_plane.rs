//! Tests for `bisect_plane`: split an `EditMesh` along a plane,
//! verify cap polygon and side classification.

use bevy::math::Vec3;
use jackdaw_geometry::BrushPlane;
use jackdaw_geometry::editmesh::EditMesh;
use jackdaw_geometry::editmesh::ops::bisect_plane::{BisectKeep, bisect_plane};
...
fn beveled_cube() -> EditMesh {
    let brush = Brush::cuboid(1.0, 1.0, 1.0);
    let mut bmesh = EditMesh::lift_from_topology(&brush.topology);
    let first_edge = bmesh.edges.keys().next().unwrap();
    edge_bevel(&mut bmesh, &[first_edge], 0.2).expect("bevel");
    bmesh.validate().expect("valid after bevel");
    bmesh
}
...
fn clip_concave_brush_keep_front_produces_correct_topology() {
    ...
    let mut bmesh = beveled_cube();
    let pre_face_count = bmesh.face_count();
    assert!(
        pre_face_count > 6,
    ...
    let result = bisect_plane(&mut bmesh, &plane, BisectKeep::Front).expect("bisect");
    bmesh.validate().expect("valid after bisect");

    // Every surviving vert is in or on +Z half-space.
    for (_, v) in bmesh.verts.iter() {
        assert!(
    ...
    for _ in 0..cap_face.loop_count {
        let p = bmesh.verts[bmesh.loops[cur].vert].co;
...
fn clip_concave_brush_keep_back_produces_correct_topology() {
    ...
    let result = bisect_plane(&mut bmesh, &plane, BisectKeep::Back).expect("bisect");
    bmesh.validate().expect("valid after bisect");
...
fn bisect_offset_plane_keeps_correct_half_space() {
    ...
    bisect_plane(&mut bmesh, &plane, BisectKeep::Front).expect("bisect");
    bmesh.validate().expect("valid after offset bisect");
    for (_, v) in bmesh.verts.iter() {
