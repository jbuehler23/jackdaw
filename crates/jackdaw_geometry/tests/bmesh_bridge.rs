use bevy::math::Vec3;
use jackdaw_geometry::bmesh::{ops::edge_create::bm_edge_create, BMesh};
use jackdaw_geometry::bmesh::ops::bridge_edge_loops::bridge_edge_loops;

fn build_two_squares_no_bridge() -> (BMesh, Vec<jackdaw_geometry::bmesh::EdgeKey>, Vec<jackdaw_geometry::bmesh::EdgeKey>) {
    ...
    let edges_b = vec![
        bm_edge_create(&mut bmesh, b0, b1),
        bm_edge_create(&mut bmesh, b1, b2),
        bm_edge_create(&mut bmesh, b2, b3),
        bm_edge_create(&mut bmesh, b3, b0),
    ];
    // Top square at z=1.
    ...
    let edges_t = vec![
        bm_edge_create(&mut bmesh, t0, t1),
        bm_edge_create(&mut bmesh, t1, t2),
        bm_edge_create(&mut bmesh, t2, t3),
        bm_edge_create(&mut bmesh, t3, t0),
    ];
...
fn bridge_unequal_loops_errors() {
    ...
    let e_a = vec![
        bm_edge_create(&mut bmesh, v0, v1),
        bm_edge_create(&mut bmesh, v1, v2),
        bm_edge_create(&mut bmesh, v2, v0),
    ];
    ...
    let e_b = vec![
        bm_edge_create(&mut bmesh, v3, v4),
        bm_edge_create(&mut bmesh, v4, v5),
        bm_edge_create(&mut bmesh, v5, v6),
        bm_edge_create(&mut bmesh, v6, v3),
    ];
