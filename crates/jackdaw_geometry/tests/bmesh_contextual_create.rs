...
fn contextual_create_with_2_unconnected_verts_creates_edge() {
    ...
    let result = contextual_create(&mut bmesh, &[v0, v1]).expect("create");
    let ContextualResult::Edge(_) = result else {
        panic!("expected edge result")
    };
    assert_eq!(bmesh.edge_count(), 1);
...
fn contextual_create_with_3_verts_creates_triangle_face() {
    ...
    let result = contextual_create(&mut bmesh, &[v0, v1, v2]).expect("create");
    let ContextualResult::Face(_) = result else {
        panic!("expected face result")
    };
    assert_eq!(bmesh.vert_count(), 3);
    assert_eq!(bmesh.edge_count(), 3);
    assert_eq!(bmesh.face_count(), 1);
...
fn contextual_create_with_4_verts_creates_quad_face() {
    ...
    let result = contextual_create(&mut bmesh, &[v0, v1, v2, v3]).expect("create");
    let ContextualResult::Face(face) = result else {
        panic!("expected face result")
    };
    assert_eq!(bmesh.face_count(), 1);
    assert_eq!(bmesh.faces[face].loop_count, 4);
    bmesh.validate().expect("valid");
}
...
fn contextual_create_with_2_verts_already_connected_returns_existing_edge() {
    ...
    let v0 = bmesh.add_vert(Vec3::ZERO);
    let v1 = bmesh.add_vert(Vec3::X);
    let result1 = contextual_create(&mut bmesh, &[v0, v1]).expect("create");
    let ContextualResult::Edge(e1) = result1 else {
        panic!("expected edge")
    };
    let result2 = contextual_create(&mut bmesh, &[v0, v1]).expect("create idempotent");
