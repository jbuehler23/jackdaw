...
pub mod headless;
pub use headless::run_headless_operator;
pub mod hot_reload;
pub mod layout;
...
impl<P: Plugin + Default> Plugin for GameSubAppInstallPlugin<P> {
    fn build(&self, app: &mut App) {
        ...
        // running inside a system, which only have `&mut World`, can
        // swap the SubApp without an editor restart. The editor's
        ...
        // instance separately.
        //
        // The editor's `AppTypeRegistry` is shared with the SubApp so
        // user-registered components (`#[derive(Reflect)]` types
        // declared by the user's plugin) become known to the editor
        // world, enabling the reverse-extract path to deserialize
        // them onto `GameMirror` entities for the Inspector to read.
        let editor_registry = app
            .world()
            .resource::<bevy::ecs::reflect::AppTypeRegistry>()
            .clone();
        let holder = jackdaw_runtime::GameSubAppHolder::new(Box::new(move || {
            let mut sub =
                jackdaw_runtime::create_game_sub_app_with_registry(Some(editor_registry.clone()));
