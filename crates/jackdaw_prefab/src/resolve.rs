...
pub struct CycleError {
    /// The sources being expanded, ending with the one that closed the loop.
...
/// Why a document could not be resolved. Every variant names the source
/// that caused it, so the warning a caller logs names the file to fix.
#[derive(Debug)]
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooDeep(p) => write!(
                f,
                "prefab references nest more than {MAX_PREFAB_DEPTH} deep at {}",
                p.display()
            ),
        }
    }
...
/// existing patch or inserting a new one. Manipulates patches directly
/// rather than going through the reflection registry, so it works for any
/// authored type path.
pub fn set_whole_component(ast: &mut SceneBsnAst, node: Entity, type_path: &str, value: BsnValue) {
...
/// Deep-copy an entire scene document into a fresh one. Alias over
/// [`SceneBsnAst::deep_clone`].
...
mod tests {
    ...
    /// A prefab whose root instances `source`, so a chain of them nests.
    fn make_link(source: &str) -> SceneBsnAst {
        let mut ast = SceneBsnAst::default();
        let root =
            ast.create_entity_node(vec![BsnPatch::Type(PREFAB_TYPE.to_string()), peid_patch(0)]);
        let child = ast.create_entity_node(vec![peid_patch(1), isa_patch(source, vec![])]);
        ast.add_to_roots(root);
        ast.add_child_to_ast(root, child);
        ast
    }
    ...
    #[test]
    fn a_prefab_that_inherits_from_itself_is_refused() {
        let mut prefabs = std::collections::HashMap::new();
        prefabs.insert(PathBuf::from("loop.bsn"), make_link("loop.bsn"));

        let mut scene = SceneBsnAst::default();
        let instance = scene.create_entity_node(vec![isa_patch("loop.bsn", vec![])]);
        scene.add_to_roots(instance);

        let get = lookup(&prefabs);
        let Err(err) = resolve_scene(&scene, &get) else {
    ...
    fn a_chain_longer_than_the_cap_is_refused() {
        // Every link names the next and the last names nothing, so only the
        // depth cap can stop the chain.
        let mut prefabs = std::collections::HashMap::new();
        for level in 0..=MAX_PREFAB_DEPTH + 2 {
            prefabs.insert(
                PathBuf::from(format!("level{level}.bsn")),
                make_link(&format!("level{}.bsn", level + 1)),
            );
        }

        let mut scene = SceneBsnAst::default();
        let instance = scene.create_entity_node(vec![isa_patch("level0.bsn", vec![])]);
        scene.add_to_roots(instance);

        let get = lookup(&prefabs);
        let Err(err) = resolve_scene(&scene, &get) else {
            panic!("the chain is refused");
        };
        assert!(matches!(err, ResolveError::TooDeep(_)), "got {err}");
    }

    #[test]
    fn a_chain_up_to_the_cap_still_resolves() {
        let mut prefabs = std::collections::HashMap::new();
        for level in 0..MAX_PREFAB_DEPTH - 1 {
            prefabs.insert(
                PathBuf::from(format!("level{level}.bsn")),
                make_link(&format!("level{}.bsn", level + 1)),
            );
        ...
        prefabs.insert(
            PathBuf::from(format!("level{}.bsn", MAX_PREFAB_DEPTH - 1)),
            make_prefab(),
        );
        ...
        let get = lookup(&prefabs);
        assert!(
            resolve_scene(&scene, &get).is_ok(),
            "the cap is a ceiling, not a lower bound"
        );
    }
