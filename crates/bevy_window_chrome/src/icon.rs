...
fn load_icon_png(png_bytes: &[u8]) -> Option<Icon> {
    let image = match image::load_from_memory(png_bytes) {
        ...
        Err(error) => {
            bevy::log::warn_once!(
                "bevy_window_chrome: failed to decode window icon PNG: {:?}",
                error
            );
            return None;
        }
    ...
    let icon = match Icon::from_rgba(rgba, width, height) {
        ...
        Err(error) => {
            bevy::log::warn_once!(
                "bevy_window_chrome: failed to create window icon from PNG: {:?}",
                error
            );
            return None;
        }
    ...
    for event in created.read() {
        WINIT_WINDOWS.with(|windows_cell| {
            ...
            let Some(backend_window) = winit_windows.get_window(event.window) else {
                bevy::log::warn_once!(
                    "bevy_window_chrome: winit backend window missing when applying icon ({:?}); ignoring",
                    event.window,
                );
                return;
            };
